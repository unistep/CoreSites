...
export class UGmapsService {
  ...
  //=================================================================================
	drawCenteredMap(mapCenter) {
		...
		this.map = new window.google.maps.Map(elmMap, map_options);
	...
	setMarkerInfoWindow(marker, onLongClickMarkerAction) {
		...
		window.google.maps.event.addListener(marker, 'mousedown', function (event) {
			start = new Date().getTime();
		});

		window.google.maps.event.addListener(marker, 'mouseup', function (event) {
	...
	getGoogleDuration(origin, destination, callback, context) {
		...
		var directions_service = new window.google.maps.DirectionsService();
		var directions_display = null;

		if (this.map) {
			directions_display = new window.google.maps.DirectionsRenderer({
